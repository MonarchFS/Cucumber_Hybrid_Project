package stepDefinations;

import org.junit.Assert;
import org.openqa.selenium.By;
import org.openqa.selenium.WebDriver;

import factory.DriverFactory;
import io.cucumber.java.en.And;
import io.cucumber.java.en.Given;
import io.cucumber.java.en.Then;
import io.cucumber.java.en.When;
import pages.HomePage;
import pages.SearchResultPage;

public class Search {
	WebDriver driver;
	
	private HomePage hp;
	private SearchResultPage srp;
	

	@Given("User is in store page")
	public void user_is_in_store_page() {
		
	   driver = DriverFactory.getDriver();
	   hp=new HomePage(driver);
	}

	@When("User enters valid product {string} into search box field")
	public void user_enters_valid_product_into_search_box_field(String valprd) {
		
	    hp.enterProductIntoSearchBox(valprd);
		//driver.findElement(By.name("search")).sendKeys(valprd);
		
	}

	@And("User clicks on search button")
	public void user_clicks_on_search_button() {
		
		srp=hp.clickOnSearchButton();
	    //driver.findElement(By.xpath("//button[contains(@class,'btn-default')]")).click();
	}

	@Then("User should get valid proiduct displayed in search result")
	public void user_should_get_valid_proiduct_displayed_in_search_result() {
	   //since this opens a new search result page so we create new pom class
		//SearchResultPage srp = new SearchResultPage(driver);
		Assert.assertTrue(srp.displayStatusOfValidProduct());
		//Assert.fail();
	}

	@When("User enters invalid product {string} into search box field")
	public void user_enters_invalid_product_into_search_box_field(String invalprd) {
		
		
		hp.enterProductIntoSearchBox(invalprd);
		driver.findElement(By.name("search")).sendKeys(invalprd);
	}

	@Then("User should get a message about no product matching")
	public void user_should_get_a_message_about_no_product_matching() {
		
		//SearchResultPage sp = new SearchResultPage(driver);
		Assert.assertEquals("There is no product that matches the search criteria.", srp.getMessageText());
	    
	}

	@When("User do not enter any product name in the search box field")
	public void user_do_not_enter_any_product_name_in_the_search_box_field() {
	    
		HomePage hp = new HomePage(driver);
	}
}
